...
(define (racketlist->mupllist xs)
  (cond [(null? xs) (aunit)]
        ...
        [#t (error "argument is not a list")]))
  ...
  (cond [(var? e) 
        ...
        [(mlet? e) (eval-under-env
                    ...
                    (cons (cons (mlet-var e) (eval-under-env (mlet-e e) env))
                          env))]
        [(call? e)
         (let ([closure-exp (eval-under-env (call-funexp e) env)]
               [arg (eval-under-env (call-actual e) env)])
           (if (closure? closure-exp)
               (let* ([fnexp (closure-fun closure-exp)]
                     [fnname (fun-nameopt fnexp)]
                     [new-env (cons (cons (fun-formal fnexp) arg)
                                    (closure-env closure-exp))])
                 (eval-under-env (fun-body fnexp)
                                 (if fnname
                                     (cons (cons fnname closure-exp) new-env)
                                     new-env)))
...
(define mupl-map (fun "mupl-map" "fn" (fun #f "xs"
    (ifaunit (var "xs")
             (aunit)
             (apair (call (var "fn") (fst (var "xs")))
                    (call (call (var "mupl-map") (var "fn"))
                          (snd (var "xs"))))))))
